...
findings.jsonl and one quiet human-facing surface").
    ...
    python3 -m oversight_console.cli --agent-uid 1001 \\
...
from oversight_console.notifier import notify
from oversight_console.run import Config, run_once


def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="oversight-console", description="Silent detector (design doc §5)."
    )
    p.add_argument(
        "--agent-uid",
        type=int,
    ...
    p.add_argument("--state", default="oversight_console_state.json",
